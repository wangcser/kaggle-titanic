...
def data_set():
    ...
    # preview the data: show the first 5 rows.
    # print(train_df.head())
    # print(train_df.tail())

    # show the data overall attribute
    # print(train_df.info())
    # print('_'*40 + '\n')
    # print(test_df.info())

    # show the static result of the data set, only deal with numerical class.
    # print(train_df.describe())

    # analyze by pivoting features
    """
    Pclass_distribution = train_df[['Pclass', 'Survived']]\
        .groupby(['Pclass'], as_index=False)\
        .mean()\
        .sort_values(by='Survived', ascending=False)
    print(Pclass_distribution)
    
    Sex_distribution = train_df[['Sex', 'Survived']]\
        .groupby(['Sex'], as_index=False)\
        .mean()\
        .sort_values(by='Survived', ascending=False)
    print(Sex_distribution)
    
    SibSp_distribution = train_df[['SibSp', 'Survived']]\
        .groupby(['SibSp'], as_index=False)\
        .mean()\
        .sort_values(by='Survived', ascending=False)
    print(SibSp_distribution)
    
    Parch_distribution = train_df[['Parch', 'Survived']]\
    ...
    # the age distribution over survived.
    g = sns.FacetGrid(train_df, col='Survived')
    g.map(plt.hist, 'Age', bins=20)
    plt.show()
    
    # the Pclass distribution over survived.
    grid = sns.FacetGrid(train_df, col='Survived', row='Pclass', size=2.2, aspect=1.6)
    grid.map(plt.hist, 'Age', alpha=.5, bins=20)
    grid.add_legend()
    plt.show()
    
    # the Embarked dark info.
